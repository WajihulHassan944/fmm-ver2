...
const emptyBout = (index = 0) => ({ boutLabel: index === 0 ? 'MAIN EVENT' : index === 1 ? 'CO-MAIN EVENT' : `FIGHT #${index + 1}`, cardSection: 'MAIN_CARD', category: 'boxing', fighterAName: '', fighterAImage: '', fighterBName: '', fighterBImage: '', pot: 0, entryTokens: 0 });
const emptyCard = { eventName: '', organizationName: '', eventPoster: '', promotionLogo: '', eventDate: '', startTime: '', venue: '', location: '', description: '', fullCardPrize: 0, bouts: [emptyBout(0)] };
...
export default function FullCardPromoterDesk() {
  ...
  useEffect(() => { load(); }, []);
  const updateBout = (index, key, value) => setForm((old) => ({ ...old, bouts: old.bouts.map((bout, i) => i === index ? { ...bout, [key]: value } : bout) }));
  ...
  const save = async (publish = false) => {
    ...
    try {
      const data = editingId
        ? await fullCardRequest(`/api/affiliates/me/full-cards/${editingId}`, { method: 'PUT', kind: 'affiliate', body: form })
        : await fullCardRequest('/api/affiliates/me/full-cards', { method: 'POST', kind: 'affiliate', body: form });
  ...
  const edit = (card) => { setEditingId(card.id); setForm({ ...emptyCard, ...card, eventDate: String(card.eventDate || '').slice(0, 10), bouts: card.bouts?.length ? card.bouts : [emptyBout(0)] }); window.scrollTo({ top: 0, behavior: 'smooth' }); };
