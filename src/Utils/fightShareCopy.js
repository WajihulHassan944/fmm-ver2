...
export const affiliateFightPosts = (title, link, league = 'my league', prize = 0, entryCoins = 0) => {
  ...
  const reward = Number(entryCoins) > 0 && Number(prize) > 0
    ? `Compete for a ${Number(prize).toLocaleString()} FM coins prize pool and eligible cash winnings.`
    : Number(prize) > 0
      ? `Compete for a ${Number(prize).toLocaleString()} FM coins prize pool and other rewards.`
  ...
  const xPrize = Number(prize) > 0 ? ` Compete for ${Number(prize).toLocaleString()} FM coins${Number(entryCoins) > 0 ? ' and eligible cash winnings' : ''}.` : ' Compete for prizes where eligible.';
