...
const safePredictions = (match) =>
  Array.isArray(match?.userPredictions) ? match.userPredictions : [];
...
const parseDashboardMatchDate = (match = {}) => {
  ...
  const rawTime =
    String(match?.matchTime || match?.time || "00:00").trim() || "00:00";
...
const getDashboardMobilePlayerCount = (match = {}) => {
  const explicit = Number(
    match?.playerCount || match?.players || match?.totalPlayers || 0,
  );
...
const getDashboardMobileFightTitle = (match = {}) =>
  match?.matchName ||
  `${getFighterName(match, "A")} vs ${getFighterName(match, "B")}`;

const getDashboardMobileImage = (match = {}, side = "A", index = 0) =>
  getFighterImage(match, side, index) ||
  MOBILE_DASHBOARD_FALLBACK_CARDS[
    index % MOBILE_DASHBOARD_FALLBACK_CARDS.length
  ];
...
const getWrestlingMobileName = (match = {}, side = "A") => {
  ...
  const competitor = isA
    ? match?.competitorA || match?.wrestlerA
    : match?.competitorB || match?.wrestlerB;
  return (
    competitor?.displayName ||
    competitor?.name ||
    (isA ? match?.wrestlerAName : match?.wrestlerBName) ||
    (isA ? "Wrestler A" : "Wrestler B")
  );
};
...
const MobileDashboardFightCard = ({ match, variant = "pending", onOpen }) => {
  ...
  return (
    <article
      className={`player-mobile-fight-card ${isCompleted ? "is-completed" : ""}`}
    >
      <button type="button" onClick={() => onOpen(match)}>
        ...
        <div className="player-mobile-fighter-row">
          <figure className="player-mobile-fighter-avatar">
            <img
              src={getDashboardMobileImage(match, "A", 0)}
              alt={getFighterName(match, "A")}
            />
          </figure>
          <b>VS</b>
          <figure className="player-mobile-fighter-avatar">
            <img
              src={getDashboardMobileImage(match, "B", 1)}
              alt={getFighterName(match, "B")}
            />
          </figure>
        </div>
        ...
        <div className="player-mobile-fight-meta">
          <span>
            <small>{isCompleted ? "Result" : "Locks in"}</small>
            <strong>
              {isCompleted ? "Review" : getDashboardMobileLock(match)}
            </strong>
          </span>
          <span>
            <small>Players</small>
            <strong>
              {getDashboardMobilePlayerCount(match).toLocaleString()}
            </strong>
          </span>
          <span>
            <small>Entry</small>
            <strong>{getDashboardMobileEntry(match)}</strong>
          </span>
        </div>
        <div className="player-mobile-card-action">
          {isCompleted ? "View Result" : "Make Picks"}{" "}
  ...
  <article className="player-mobile-contest-row">
    <button type="button" onClick={() => onOpen(match)}>
      <div className="player-mobile-contest-visual">
        <span>
          {index === 0 ? "Live" : index === 1 ? "Featured" : "Top Prize"}
        </span>
        <div>
          <figure className="player-mobile-fighter-avatar">
            <img
              src={getDashboardMobileImage(match, "A", index)}
              alt={getFighterName(match, "A")}
            />
          </figure>
          <figure className="player-mobile-fighter-avatar">
            <img
              src={getDashboardMobileImage(match, "B", index + 1)}
              alt={getFighterName(match, "B")}
            />
          </figure>
        </div>
      ...
      <div className="player-mobile-contest-copy">
        ...
        <div>
          <span>
            ...
            <strong>{getDashboardMobilePrize(match)}</strong>
          </span>
          <span>
            <small>Players</small>
            <strong>
      ...
      <Link href={href}>
        <div className="player-mobile-contest-visual">
          ...
          <div>
            <figure className="player-mobile-fighter-avatar">
              <img
                src={getPWImage(match?.competitorA || match?.wrestlerA, "A")}
                alt={getWrestlingMobileName(match, "A")}
              />
            </figure>
            <figure className="player-mobile-fighter-avatar">
              <img
                src={getPWImage(match?.competitorB || match?.wrestlerB, "B")}
                alt={getWrestlingMobileName(match, "B")}
              />
            </figure>
          </div>
        </div>
        <div className="player-mobile-contest-copy">
          ...
          <div>
            ...
            <span>
              <small>Status</small>
              <strong>{match?.status || "Open"}</strong>
            </span>
          </div>
        </div>
...
const MobilePlayerDashboard = ({
  ...
  const activeSection =
    categorySections.find((section) => section.key === activeSport) ||
    categorySections[0];
  const selectedFights = activeSection?.fights?.length
    ? activeSection.fights.slice(0, 5)
    : dashboardOpportunities.slice(0, 5);
  const primaryMission = nextMission || selectedFights[0] || null;
  const contestRows = pendingMatches.length
    ? pendingMatches.slice(0, 3)
    : dashboardOpportunities.slice(0, 3);
  const visibleWrestlingRows = Array.isArray(wrestlingMatches)
    ? wrestlingMatches.slice(0, 3)
    : [];
  const visibleCompleted = completedMatches.slice(0, 3);

  return (
    <div
      className="player-command-mobile-app"
      aria-label="Fantasy MMAdness mobile user dashboard"
    >
      <section className="player-mobile-hero-card">
        ...
        <div className="player-mobile-profile-row">
          <img
            src={
              user.profileUrl || "/images/fmm-experience/avatar-placeholder.svg"
            }
            alt={user.firstName}
          />
          <span>
        ...
        {primaryMission ? (
          <div className="player-mobile-featured-mission">
            <span className="player-mobile-featured-label">
              Next Prediction
            </span>
            <div className="player-mobile-featured-fighters">
              <figure className="player-mobile-fighter-avatar is-large">
                <img
                  src={getDashboardMobileImage(primaryMission, "A", 0)}
                  alt={getFighterName(primaryMission, "A")}
                />
              </figure>
              <b>VS</b>
              <figure className="player-mobile-fighter-avatar is-large">
                <img
                  src={getDashboardMobileImage(primaryMission, "B", 1)}
                  alt={getFighterName(primaryMission, "B")}
                />
              </figure>
            </div>
            <h2>
              {getFighterName(primaryMission, "A")} <em>vs</em>{" "}
              {getFighterName(primaryMission, "B")}
            </h2>
            <div className="player-mobile-countdown-row">
              <span>
                <small>Date</small>
                <strong>{formatDashboardMobileDate(primaryMission)}</strong>
              </span>
              <span>
                <small>Locks</small>
                <strong>{getDashboardMobileLock(primaryMission)}</strong>
              </span>
              <span>
                <small>Entry</small>
                <strong>{getDashboardMobileEntry(primaryMission)}</strong>
              </span>
            </div>
            <button
              type="button"
              className="player-mobile-primary-btn"
              onClick={() => onPendingOpen(primaryMission)}
            >
          ...
          <div className="player-mobile-empty-panel">
            ...
            <span>
              New prediction cards will appear here as soon as they are opened.
            </span>
      ...
      <section
        className="player-mobile-stat-grid"
      ...
      >
        ...
        <article>
          ...
          <strong>{currentRankEstimate}</strong>
          <span>Momentum Rank</span>
        </article>
      </section>

      <section
        className="player-mobile-category-row"
        aria-label="Fight categories"
      >
      ...
      <section className="player-mobile-section">
        <div className="player-mobile-section-heading">
          ...
          <Link
            href={`/upcomingfights?status=all&category=${encodeURIComponent(activeSection?.key || "all")}`}
          >
        ...
        <div className="player-mobile-fight-rail">
          {selectedFights.length ? (
            selectedFights.map((match, index) => (
              <MobileDashboardFightCard
            ...
            <div className="player-mobile-inline-empty">
              No {activeSection?.label || "fight"} cards are open right now.
            </div>
          )}
        </div>
      </section>
      ...
      <section className="player-mobile-section">
        <div className="player-mobile-section-heading">
          <h2>Open Contests</h2>
          <Link href="/YourFights">
            View All <FaArrowRight aria-hidden="true" />
          </Link>
        </div>
        <div className="player-mobile-contest-list">
          {contestRows.length ? (
            contestRows.map((match, index) => (
              <MobileDashboardContestRow
                key={getFightId(match) || `contest-${index}`}
                match={match}
                index={index}
                onOpen={onPendingOpen}
              />
      ...
      <section className="player-mobile-section">
        <div className="player-mobile-section-heading">
          ...
          <Link href="/YourFights">
            Library <FaArrowRight aria-hidden="true" />
          </Link>
        </div>
      ...
      <section className="player-mobile-wrestling-banner">
        <div>
          ...
          <p>
            {wrestlingHistory.length} wrestling entries saved. Predict power
            moves, finishers, and winners from the dedicated lobby.
          </p>
        ...
        <section className="player-mobile-section">
          ...
          <div className="player-mobile-contest-list">
            {visibleWrestlingRows.map((match, index) => (
              <MobileWrestlingContestRow
                key={match?._id || `wrestling-${index}`}
                match={match}
                index={index}
              />
            ))}
      ...
      <section className="player-mobile-section">
        ...
        <div className="player-mobile-completed-list">
          {visibleCompleted.length ? (
            visibleCompleted.map((match, index) => (
              <MobileDashboardFightCard
                key={getFightId(match) || `completed-${index}`}
                match={match}
                variant="completed"
            ...
            <div className="player-mobile-inline-empty">
              Submitted prediction cards will appear here.
            </div>
          )}
        </div>
      </section>
      ...
      {!user.hasSubmittedTestimonial && (
        <button
          className="player-mobile-testimonial"
          type="button"
          onClick={onTestimonialOpen}
        >
          <FaStar aria-hidden="true" />
          <span>
            <strong>Share your experience</strong>
            <small>Your story may be featured publicly.</small>
          </span>
          <FaArrowRight aria-hidden="true" />
        </button>
      )}
...
const isPredictionStatusSubmitted = (value) =>
  [
    "submitted",
    "complete",
    "completed",
...
const hasEmbeddedSubmittedPrediction = (match, userId) =>
  Boolean(
    userId &&
    safePredictions(match).some(
      (prediction) =>
        isSameId(prediction?.userId, userId) &&
        isPredictionStatusSubmitted(prediction?.predictionStatus),
    ),
  );
const isSubmittedByUser = (match, userId) => {
  const status = String(
    match?.userPredictionStatus || match?.predictionStatus || "",
  ).toLowerCase();
...
const mergeFightRowsForUser = (rows = [], userId) => {
  ...
  (Array.isArray(rows) ? rows : []).filter(Boolean).forEach((match) => {
    ...
    const userPredictions =
      nextPredictions.length >= existingPredictions.length
        ? nextPredictions
        : existingPredictions;
    const preferred =
      nextSubmitted ||
    ...
    byId.set(id, {
      ...
      userPredictionStatus: submitted
        ? "submitted"
        : preferred.userPredictionStatus ||
          secondary.userPredictionStatus ||
          "not_submitted",
      userFightBucket: submitted
        ? "completed"
        : preferred.userFightBucket || secondary.userFightBucket || "",
      fightStatusBucket:
        preferred.fightStatusBucket || secondary.fightStatusBucket,
      canSubmitPrediction: submitted
        ? false
        : (preferred.canSubmitPrediction ?? secondary.canSubmitPrediction),
    });
  });
...
const Dashboard = () => {
    ...
    const fetchRemovedMatches = async () => {
      try {
        ...
        const userMatches = Array.isArray(data)
          ? data.filter((item) => isSameId(item.userId, user._id))
          : [];
  ...
  useEffect(() => {
    ...
    let active = true;
    Promise.all([
      wrestlingRequest("/api/users/me/wrestling-history?limit=6", {
        auth: true,
      }),
      wrestlingRequest(
        "/api/wrestling/matches?limit=4&status=OPEN,LIVE,SCORING",
      ),
    ])
      ...
      .catch((requestError) => {
        console.info(
          "Pro Wrestling dashboard module is not available yet:",
  ...
  useEffect(() => {
    const sourceRows = playableMatches.length
      ? playableMatches
  ...
  const getRemainingTime = (matchDate, matchTime) => {
    const [year, month, day] = String(matchDate || "")
      .split("T")[0]
      .split("-");
  ...
  const allFightRows = useMemo(
    ...
    [matches, playableMatches, user?._id],
  );

  const completedMatches = useMemo(
    () =>
      orderFightsForDisplay(
        allFightRows.filter((match) => isSubmittedByUser(match, user?._id)),
      ),
    [allFightRows, user?._id],
  );
  const pendingMatches = useMemo(
    ...
    [removedMatches, upcomingMatches, user?._id],
  );
  const visibleCompleted = useMemo(
    () =>
      orderFightsForDisplay(
        completedMatches.filter(
          (match) => !removedMatches.includes(getFightId(match)),
        ),
      ),
    [completedMatches, removedMatches],
  );
  const dashboardOpportunities = useMemo(() => {
    const seen = new Set();
    const merged = orderFightsForDisplay([
      ...pendingMatches,
      ...upcomingMatches,
    ]).filter((match) => {
      const id = getFightId(match);
  ...
  const mobileDashboardSections = useMemo(() => {
    ...
    return MOBILE_DASHBOARD_SPORT_TABS.map((tab) => {
      const fights = uniqueRows.filter(
        (match) => getFightSportKey(match) === tab.key,
      );
      return {
        ...tab,
        count: fights.length,
  ...
  const handleRemoveMatch = async (matchId) => {
    try {
      const response = await fetch(
        "https://fantasymmadness-game-server-three.vercel.app/remove-match-from-my-dashboard",
        {
          method: "POST",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "https://fantasymmadness-game-server-three.vercel.app/testimonials",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  if (!user?.firstName) {
    return (
      <section className="player-command-page">
        <div className="theme-container">
          <div className="player-dynamic-empty">
            <FaFistRaised />
            <h2>Loading player command center…</h2>
          </div>
        </div>
      </section>
    );
  ...
  if (selectedMatchId) {
    const selectedMatch = allFightRows.find(
      (match) => getFightId(match) === selectedMatchId,
    );
    if (!selectedMatch) {
      return (
        <section className="player-dynamic-route-view">
          <div className="theme-container player-dynamic-back-row">
            <button type="button" onClick={() => setSelectedMatchId(null)}>
              <FaArrowLeft /> Back to dashboard
            </button>
          </div>
          <div className="player-dynamic-empty">
            <FaShieldAlt />
            <h2>Selected fight not found</h2>
          </div>
        </section>
      );
    }
  ...
  if (completedMatchId) {
    const completedMatch = allFightRows.find(
      (match) => getFightId(match) === completedMatchId,
    );
    if (!completedMatch) {
      return (
        <section className="player-dynamic-route-view">
          <div className="theme-container player-dynamic-back-row">
            <button type="button" onClick={() => setCompletedMatchId(null)}>
              <FaArrowLeft /> Back to dashboard
            </button>
          </div>
          <div className="player-dynamic-empty">
            <FaShieldAlt />
            <h2>Completed fight not found</h2>
          </div>
        </section>
      );
    }
  ...
  const renderFightCard = (match, variant = "active") => {
    ...
    const remaining =
      match.matchType === "LIVE"
        ? getRemainingTime(match.matchDate, match.matchTime)
        : null;
    ...
    const handleOpen = () => {
      ...
      if (match.matchType === "SHADOW" && match.blurred)
        toast.error(
          "Affiliate criteria has not been met for this SHADOW match.",
        );
    ...
    return (
      ...
      >
        {hoveredMatch === id && (
          <button
            className="player-command-remove"
            type="button"
            onClick={(event) => {
        ...
        <button type="button" onClick={handleOpen} disabled={!canOpen}>
          <div className="player-command-fight-art">
            <figure>
              <img
                src={getFighterImage(match, "A", 0)}
                alt={getFighterName(match, "A")}
              />
              <figcaption>{getFighterName(match, "A")}</figcaption>
            </figure>
            <span>VS</span>
            <figure>
              <img
                src={getFighterImage(match, "B", 1)}
                alt={getFighterName(match, "B")}
              />
              <figcaption>{getFighterName(match, "B")}</figcaption>
            </figure>
          </div>
          <div className="player-command-fight-copy">
            ...
            <h3>
              {getFighterName(match, "A")} <em>vs</em>{" "}
              {getFighterName(match, "B")}
            </h3>
            <div>
              <span>
                <FaCalendarAlt /> {match.matchDate?.split("T")[0] || "Date TBA"}
              </span>
              <span>
                <FaUsers /> {predictionsCount} players
              </span>
              <span>
                <FaCoins />{" "}
            ...
            {remaining && (
              <span>
                {remaining.hasStarted
                  ? "Fight has started"
                  : `${remaining.diffHrs}h ${remaining.diffMins}m to lock`}
              </span>
            )}
            <strong>
              {variant === "completed" ? "View result" : "Make predictions"}{" "}
              <FaArrowRight />
            </strong>
          </div>
        </button>
  ...
  const currentRankEstimate = visibleCompleted.length
    ? Math.max(1, 100 - visibleCompleted.length * 3)
    : "—";
  const quickActions = [
    {
      href: "/YourFights",
      label: "My fight library",
      copy: "All completed and pending cards",
      icon: FaFistRaised,
    },
    {
      href: "/pro-wrestling",
      label: "Pro Wrestling",
      copy: "Full-match action prediction mode",
      icon: FaCrown,
    },
    {
      href: "/myLeagueRecords",
      label: "My leagues",
      copy: "Creator communities and records",
      icon: FaUsers,
    },
    {
      href: "/fighter-performance-tracker",
      label: "Fighter tracker",
      copy: "Research form and performance",
      icon: FaChartLine,
    },
    {
      href: "/account-settings",
      label: "Account settings",
      copy: "Preferences and payment details",
      icon: FaUserCog,
    },
  ];
    ...
    <div className="player-command-page">
      ...
      <div className="player-command-desktop-shell">
        <section className="player-command-hero">
          ...
          <div className="theme-container player-command-hero-layout">
            <div className="player-command-hero-copy">
              <p>
                <FaFire /> Player command center
              </p>
              <h1>
                Read the card. <span>Own the round.</span>
              </h1>
              <p>
                Welcome back, {user.firstName}. Your dashboard now brings every
                pending pick, completed result, league, wallet, and research
                tool into one premium fight-night workspace.
              </p>
              <div className="player-command-hero-actions">
                {nextMission ? (
                  <button
                    type="button"
                    onClick={() => setSelectedMatchId(getFightId(nextMission))}
                  >
                    Make next prediction <FaArrowRight />
                  </button>
                ) : (
                  <Link href="/upcomingfights">
                    Explore upcoming fights <FaArrowRight />
                  </Link>
                )}
                <Link href="/mock-game" className="is-secondary">
                  Practice in mock game <FaBolt />
                </Link>
              </div>
              <div className="player-command-hero-proof">
                <span>
                  <FaShieldAlt />
                  <strong>Original flow intact</strong>
                  <small>
                    Every API and prediction action remains connected.
                  </small>
                </span>
                <span>
                  <FaTrophy />
                  <strong>{visibleCompleted.length} submitted</strong>
                  <small>Your completed prediction cards.</small>
                </span>
              </div>
            </div>
            ...
            <aside className="player-command-spotlight">
              <header>
                <img
                  src={
                    user.profileUrl ||
                    "/images/fmm-experience/avatar-placeholder.svg"
                  }
                  alt={user.firstName}
                />
                <span>
                  <small>Fight identity</small>
                  <strong>
              ...
              {nextMission ? (
                <div className="player-command-next-card">
                  ...
                  <button
                    ...
                    onClick={() => setSelectedMatchId(getFightId(nextMission))}
                  >
                    Open scorecard <FaArrowRight />
                  </button>
                </div>
              ) : (
                <div className="player-command-next-empty">
                  <FaTrophy />
                  <h2>You are caught up.</h2>
                  <p>No pending cards require a prediction right now.</p>
                </div>
              )}
            </aside>
          </div>
        ...
        <main className="theme-container player-command-main">
          <section
            className="player-command-fight-night-strip"
            aria-label="Fresh fight opportunities"
          >
            <div>
              <p>
                <FaBolt /> Fresh fight opportunities
              </p>
              <h2>
                Tonight, live, featured, and newly updated fights stay in front.
              </h2>
              <span>
                Your dashboard keeps changing with new MMA, Boxing, and Pro
                Wrestling opportunities so there is always something fresh to
                play.
              </span>
            </div>
            <div className="player-command-fight-night-actions">
              <Link
                href="/upcomingfights"
                className="player-command-play-tonight"
              >
                <FaFistRaised /> Play tonight
              </Link>
              <Link href="/calendar-of-fights">
                <FaCalendarAlt /> Fight calendar
              </Link>
              <Link href="/pro-wrestling">
                <FaCrown /> Pro wrestling
              </Link>
              <Link href="/fights-news">
                <FaGlobe /> Fight news
              </Link>
            </div>
          </section>
          <section className="player-command-stat-grid">
            <article>
              ...
              <span>
                <strong>{pendingMatches.length}</strong>
                <small>Pending picks</small>
              </span>
            </article>
            <article>
              <FaTrophy />
              <span>
                <strong>{visibleCompleted.length}</strong>
                <small>Submitted cards</small>
              </span>
            </article>
            <article
            ...
            <article
              onClick={() => router.push("/pro-wrestling/history")}
              role="button"
              tabIndex={0}
            >
              <FaCrown />
              <span>
                <strong>{wrestlingHistory.length}</strong>
                <small>Wrestling entries</small>
              </span>
            </article>
          </section>
          ...
          <section className="player-command-quick-section">
            <header>
              <div>
                <p>Fight-night tools</p>
                <h2>Move faster from one corner.</h2>
              </div>
              <Link href="/YourFights">
                Open full fight library <FaArrowRight />
              </Link>
            </header>
          ...
          <section className="player-command-section player-command-opportunity-section">
            <header>
              <span>01</span>
              <div>
                <p>New fight opportunities</p>
                <h2>Fresh cards to play now</h2>
                <small>
                  Featured, live, tonight, newly added, and recently updated
                  fights rotate to the top of your dashboard.
                </small>
              </div>
              <Link href="/upcomingfights">
            ...
            <div className="player-command-fight-grid">
              {dashboardOpportunities.length ? (
                dashboardOpportunities.map((match) =>
                  renderFightCard(match, "pending"),
                )
              ) : (
                <div className="player-dynamic-empty is-inline">
                  <FaShieldAlt />
                  <h3>No fresh fights yet</h3>
                  <p>
                    New fight opportunities will appear here as soon as they are
                    published.
                  </p>
                </div>
              )}
          ...
          <section className="player-command-wrestling-feature">
            <div className="player-command-wrestling-copy">
              <p>
                <FaCrown /> Additional game mode
              </p>
              <h2>Pro Wrestling is now inside your command center.</h2>
              <span>
                Predict head punches, body punches, kicks, power moves,
                finishers, and the official winner across the full match.
              </span>
              <div>
                <Link href="/pro-wrestling" className="pw-btn pw-btn-primary">
                  Open wrestling lobby <FaArrowRight />
                </Link>
                <Link
                  href="/pro-wrestling/history"
            ...
            <div className="player-command-wrestling-card">
              {wrestlingMatches[0] ? (
                  ...
                  <div>
                    <figure>
                      <img
                        ...
                        alt={wrestlingMatches[0].competitorA?.displayName}
                      />
                      <figcaption>
                    ...
                    <figure>
                      <img
                        ...
                        alt={wrestlingMatches[0].competitorB?.displayName}
                      />
                      <figcaption>
                ...
                <div className="player-command-wrestling-empty">
                  <FaCrown />
                  <strong>Wrestling cards will appear here.</strong>
                  <span>
                    The public game-mode guide and roster are already available.
                  </span>
                </div>
              )}
            </div>
          ...
          <section className="player-command-section">
            <header>
              <span>02</span>
              <div>
                <p>Submitted fight cards</p>
                <h2>Your completed fights</h2>
                <small>
                  Open a card to review live standings or the completed
            ...
            <div className="player-command-fight-grid">
              {visibleCompleted.length ? (
                visibleCompleted.map((match) =>
                  renderFightCard(match, "completed"),
                )
              ) : (
                <div className="player-dynamic-empty is-inline">
                  <FaTrophy />
                  <h3>No completed matches</h3>
                  <p>Your submitted prediction cards will appear here.</p>
                </div>
              )}
            </div>
          </section>
          ...
          <section className="player-command-section">
            <header>
              <span>03</span>
              <div>
                <p>Cards requiring action</p>
                <h2>Your pending fights</h2>
                <small>
                  Open a card to review entry details and submit your
                  predictions.
                </small>
              </div>
            </header>
            <div className="player-command-fight-grid">
              {pendingMatches.length ? (
                pendingMatches.map((match) => renderFightCard(match, "pending"))
              ) : (
                <div className="player-dynamic-empty is-inline">
                  <FaShieldAlt />
                  <h3>No prediction-ready fights yet</h3>
                  <p>
                    Fresh playable fight cards will appear here as soon as admin
                    opens predictions.
                  </p>
                </div>
              )}
          ...
          {!user.hasSubmittedTestimonial && (
            <button
              className="player-command-testimonial"
              type="button"
              onClick={() => setIsOpen(true)}
            >
              <FaStar />
              <span>
                <strong>Share your Fantasy MMAdness experience</strong>
                <small>
                  Your approved story may be featured on the public testimonials
                  page.
                </small>
              </span>
              <FaArrowRight />
