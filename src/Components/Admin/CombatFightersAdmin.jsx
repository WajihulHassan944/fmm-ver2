...
export default function CombatFightersAdmin() {
  ...
  const runImport = async (dryRun = true) => {
    if (!dryRun) {
      const confirmed = window.confirm('Import unique fighters from existing fights and link matches to fighter records? This keeps old fight fields as fallback.');
      if (!confirmed) return;
    }
    setImporting(true);
    try {
      const payload = await combatFightersApi.importFromFights({
        dryRun,
        checkImages: true,
        includeShadows: true,
        linkMatches: true,
      ...
      if (dryRun) toast.success(`${payload?.fighterCount || 0} unique fighters detected. Review and run import when ready.`);
      else {
        toast.success(`${payload?.createdCount || 0} fighters created, ${payload?.updatedCount || 0} updated, ${(payload?.linkedMatchCount || 0) + (payload?.linkedShadowCount || 0)} fight links updated.`);
        await loadFighters();
      }
    } catch (error) {
    ...
    <div className="admin-workspace admin-combat-fighters-page">
      ...
      <section className="admin-page-heading">
        ...
        <div className="admin-heading-actions">
          ...
          <button type="button" className="admin-primary-action" onClick={() => runImport(false)} disabled={importing}><FaRedo /> Run automatic import</button>
      ...
      {importPreview && (
        <section className="admin-success-panel admin-fighter-import-panel">
          <div>
            <span>{importPreview.dryRun ? 'Dry-run import ready' : 'Import completed'}</span>
            <strong>{importPreview.fighterCount || importPreview.created || 0} fighter records processed</strong>
