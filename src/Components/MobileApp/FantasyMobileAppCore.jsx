...
class FantasyMobileAppCore extends React.Component {
  ...
  saveStreak = async () => {
    ...
    this.playBell();
    this.setState(s => ({ coins: Number.isFinite(Number(result.coins)) ? Number(result.coins) : s.coins - cost, streakExpiresIn: toSafeNumber(result.streakExpiresIn, 24 * 3600) }));
  ...
  renderHero() {
    return React.createElement('div', { style: { position: 'relative', width: '100%', overflow: 'hidden', background: '#000' } },
      React.createElement('img', { src: `${ASSET_BASE}/hero-banner-new.jpg`, alt: 'Fantasy MMAdness combat prediction game', style: { width: '100%', height: 'auto', display: 'block' } }),
      ...
      React.createElement('div', { onClick: () => this.openModal('join'), role: 'button', tabIndex: 0, 'aria-label': 'Join Fantasy MMAdness free', style: { position: 'absolute', left: '22%', right: '22%', bottom: '4%', height: '13%', cursor: 'pointer' } })
    );
  }
    ...
    return React.createElement('div', {
      ...
      React.createElement('div', { style: { position: 'relative', minHeight: 190, padding: 14, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', textAlign: 'center' } },
        ...
        React.createElement('div', { style: { display: 'flex', width: '100%', maxWidth: 330, gap: 7, alignItems: 'stretch' } },
          event.aiScoutingReport && React.createElement('div', {
        ...
        event.aiScoutingReport && React.createElement('span', { style: { marginTop: 5, color: '#9bbcff', fontSize: 8, fontWeight: 900, letterSpacing: .35 } }, 'AI SCOUTING REPORT')
      )
    );
  }
      ...
      React.createElement('div', { style: { display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 } },
        ...
        filteredEvents.map(ev => React.createElement('div', {
          ...
          React.createElement('div', { style: { padding: '0 10px 10px', display: 'flex', flexDirection: 'column', gap: 4 } },
            ...
            ev.aiScoutingReport && React.createElement('div', {
  ...
  renderModal(s, events, jonesPct, aspinallPct) {
    ...
    if (s.modal === 'affiliate') return overlay([
      ...
      React.createElement('div', { key: 'dashboard', onClick: () => this.props.onOpenAffiliateDashboard?.(), style: { textAlign: 'center', padding: '11px 0', borderRadius: 999, border: '1px solid rgba(242,181,68,.45)', color: '#f2b544', fontWeight: 900, fontSize: 12, cursor: 'pointer', marginBottom: 14 } }, 'VIEW MY DASHBOARD'),
